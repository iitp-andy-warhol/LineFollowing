...
def follower():
    ...
    for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
        ...
        # Stop sign handler
        if not mmode_flag:  # Imaging break in M-mode
            if len(contours_red) > 0:
                address0_time = time.time() - start_time
                print("stopsign: ", address0_time)
                # receive_path_flag = True
            # Delaying stop to stop on the sign
            if address0_time + 0.3 < time.time() - start_time < address0_time + 0.5:
                address = 0
                start = False
                # stop0 = True
                # msg0 = True
